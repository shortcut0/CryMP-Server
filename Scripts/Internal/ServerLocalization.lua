...
ServerLocale.Init = function(self)
    ...
    ServerLog("Loaded %d Localizations", table.count(aLocalizationData))
...
ServerLocale.CreateLocalization = function(self, sId, aLanguages)
    ...
    for sLang, sText in pairs(aLanguages) do
        self.Localization[sId][sLang] = sText
    end
end
            ...
            if (string.len(sData) > 0) then
                if (sType == "lua") then
                    ...
                    if (not bOk) then
                        ...
                        ServerLogError("Failed to read Locale file %s (%s)", ServerLFS.FileGetName(sFile), g_ts(sErr))
                    end
